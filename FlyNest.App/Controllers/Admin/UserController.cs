...
public class UserController(UserManager<User> userManager, RoleManager<Role> roleManager) : Controller
{
    ...
    public async Task<IActionResult> Index(int? pageNumber, int? pageSize)
    {
        ...
        var rs = await PaginatedList<User>.CreateFromEfQueryableAsync(
            users.AsNoTracking(),
            pageNumber ?? 1,
            pageSize ?? 12);
        var userViewModels = rs.Select(
            user => new UserViewModel { Id = user.Id, UserName = user.UserName, Email = user.Email })
            .ToList();
