...
public static class DropdownExtension
{
    ...
    public static async Task<Dropdown<TResult>> DropdownAsync<T, TResult>(
        ...
        int size)
    {
        ...
        return new Dropdown<TResult>() { Size = size, Data = list };
    }

    public static Dropdown<TDestination> ToDropdownModel<TSource, TDestination>(
        this Dropdown<TSource> list,
        IMapper mapper)
    {
    ...
    public static Paging<TDestination> ToDropdownModel<TDestination>(this Dropdown<object> list, IMapper mapper)
