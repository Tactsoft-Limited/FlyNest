...
public static class PagingExtention
{
    ...
    public static async Task<Paging<TResult>> PagingAsync<T, TResult>(
        ...
        int pageSize = 10)
    {
        ...
        return new Paging<TResult>() { Total = total, Data = list, PageIndex = pageIndex, PageSize = pageSize };
    }

    public static Paging<TResult> Paging<T, TResult>(
        this List<T> query,
        Expression<Func<T, TResult>> selector,
        int pageIndex = 0,
        int pageSize = 10)
    ...
    public static Paging<TDestination> ToPagingModel<TDestination>(this Paging<object> list, IMapper mapper)
